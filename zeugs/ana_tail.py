...
def main():
    ...
    data = gv.dataset.avg_data(ense[:, iflow, tau, :])


    # plot of correlator for all r
    plot_dist(dist, data, axes[0])
    ...
    fit = lsqfit.nonlinear_fit(
        data = (dist[ir_cut:], data[ir_cut:]),
        fcn  = f,
        p0   = {'A':-1, 'm':1},
        debug = True
    )
    print(fit)
    plot_fit(dist, data, ir_cut, fit, axes[1])


    # r correlations
    img = axes[2].imshow(gv.evalcorr(data), vmin=0, vmax=1)
    fig.colorbar(img, ax=axes[2], label='Correlation')
...
def get_data_binned(bin_size : float) -> np.ndarray:
    ...
    return bin_by_distance_ensemble(
        ...
        ensemble  = np.load(Path.cwd() / 'zeugs' / 'data' / 'data0.npy')[skip_configs+2000:],
        bin_size  = bin_size
    )
